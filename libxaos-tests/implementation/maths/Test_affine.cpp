...
TEST_CASE("MATHS:affine | 2D Scaling", "[maths]") {
    ...
    FloatMatrix3 scale1 = get2DScaling(2, 2);
...
TEST_CASE("MATHS:affine | 3D Scaling", "[maths]") {
    ...
    FloatMatrix4 scale1 = get3DScaling(2, 2, 2);
    FloatMatrix4 scale2 = get3DScaling(FloatVector3{3, 5, 7});
...
TEST_CASE("MATHS:affine | 3D Rotation Simple", "[maths]") {
    ...
    FloatMatrix4 rot1 = get3DRotation(0, 0, ninety);
    FloatMatrix4 rot2 = get3DRotation(FloatVector3{ninety, 0, 0});
    FloatMatrix4 rot3 = get3DRotation(0, ninety, 0);
...
TEST_CASE("MATHS:affine | 3D Rotation Onto", "[maths]") {
    ...
    REQUIRE((result.x() - 1 < 0.001F));
    REQUIRE((result.y() - 2 < 0.001F));
    REQUIRE((result.z() - 3 < 0.001F));
    REQUIRE((result.w() - 1 < 0.001F));
}
...
TEST_CASE("MATHS:affine | 2D Translation", "[maths]") {
    ...
    FloatMatrix3 trans1 = get2DTranslation(5, -5);
...
TEST_CASE("MATHS:affine | 3D Translation", "[maths]") {
    ...
    FloatMatrix4 trans1 = get3DTranslation(5, -5, 5);
