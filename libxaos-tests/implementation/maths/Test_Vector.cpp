...
// Define a few types
using IntVector1 = libxaos::physics::Vector<int, 1>;
using IntVector2 = libxaos::physics::Vector<int, 2>;
using IntVector3 = libxaos::physics::Vector<int, 3>;
using IntVector4 = libxaos::physics::Vector<int, 4>;
using FloatVector = libxaos::physics::Vector<float, 3>;
...
TEST_CASE("MATHS:Vector | Vector Magnitudes", "[maths]") {
    ...
    REQUIRE(magnitude(vec3) == sqrt(1.5F * 1.5F + 2.5F * 2.5F + 3.5F * 3.5F));
}
...
TEST_CASE("MATHS:Vector | Cross Product", "[maths]") {
    IntVector3 vec1 {1, 3, 5};
    IntVector3 vec2 {2, 4, 6};
    IntVector3 cross = cross(vec1, vec2);

    REQUIRE(cross == IntVector3{-2, 4, -2});
}
